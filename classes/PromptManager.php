...
class PromptManager
{
    ...
    {
        $existe = Db::getInstance()->getValue("SELECT COUNT(*) FROM "._DB_PREFIX_."openai_prompts WHERE grupo = '".pSQL($grupo)."' AND nombre = '".pSQL($nombre)."' AND activo = 1");
    ...
    {
        $original = new OpenAIPrompt((int)$id_prompt);
        if (!Validate::isLoadedObject($original)) {
            throw new Exception("Prompt no encontrado.");
        }

        $nombreBase = $original->nombre;
        ...
        return OpenAIPrompt::duplicarPrompt($id_prompt, $nuevoNombre, $usuario);
    }
